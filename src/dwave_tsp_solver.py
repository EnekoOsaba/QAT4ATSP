...
#### FUNCIONES PARA RESOLVER EL PROBLEMA QUBO ####
    ...
    def solve_tsp_QBSolv_DWAVE(self):
        ...
        sampler = EmbeddingComposite(DWaveSampler(token=self.sapi_token, endpoint=self.url, solver='Advantage_system1.1'))
        response = QBSolv().sample_qubo(self.qubo_dict, solver=sampler)             
    ...
    def solve_tsp_DWAVE(self):
        ...
        response = EmbeddingComposite(DWaveSampler(token=self.sapi_token, endpoint=self.url, solver='Advantage_system1.1')).sample_qubo(self.qubo_dict)             
