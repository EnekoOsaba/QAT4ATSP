...
#            print('currentRoute', currentRoute)
            getLinkToFollowingCluster(veryInitialEdge, copy.deepcopy(linkedClustersIdx), clusterX_0X_1Edge, clustersTSP_nodes, distances_df, clusters_dict, cyclesList, currentRoute)
...
def composeCycle (clustersTSP_nodes, distances_df, clusters_dict):
    ...
    for cycle in totalFinalCycleList:
        ...
        for cluster in clustersTSP_nodes:
            dist += np.sum([distances_df.loc[edge[0],edge[1]] for edge in cluster if (edge not in brokenEdges) and ( (edge[1],edge[0]) not in brokenEdges)])
            completeTSP.extend([edge for edge in cluster if (edge not in brokenEdges) and ( (edge[1],edge[0]) not in brokenEdges)])
