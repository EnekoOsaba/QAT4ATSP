...
def reduc(distances_df, points, wholePopulation, metricsToApply, subPopulationSize = subPopulationSize, end = False): 
    newPopulation = []    
    if end:
...
def solvePartitionProblem(cluster_size,tsp_matrix,node_array):
    ...
    for _ in range(generations):  
        ...
        for subPopulationIdx in range(len(metricsToApply)): # 3 subpopulations, concatenate subpopulations from pupulations and get fitness
            ...
            concat = np.concatenate((subPopulation, newSubpopulation_twoopt, newSubpopulation_insertion, newSubpopulation_swap))
            newSubPopulation, bestScore = reduc(distances_df, points, concat, metricsToApply[subPopulationIdx], subPopulationSize) 
    ...
    winners, bestScores = reduc(distances_df, points, population, metricsToApply, subPopulationSize, True) 
